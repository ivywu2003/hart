...
template<typename scalar_t>
kernel void fused_rope_backward(constant scalar_t* src [[buffer(0)]],
                              constant float* freqs [[buffer(1)]],
    ...
    for (int d_id = tid.x; d_id < d2; d_id += blockDim.x) {
        ...
        v_sin = (d_id + d2/2 < d2) ? 
            sin(freqs[s_id * d2 + d_id + d2/2]) :
            -sin(freqs[s_id * d2 + d_id + d2/2 - d2]);
        v_cos = cos(freq);
...
kernel void fused_rope_backward<float>(constant float* src [[buffer(0)]],
                              constant float* freqs [[buffer(1)]],
...
kernel void fused_rope_backward<half>(constant half* src [[buffer(0)]],
                              constant float* freqs [[buffer(1)]],
